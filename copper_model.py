...
if selected=='HOME':
    ...
    with b2:
        ...
        st.write('#### REGRESSION - ***:red[Decision Tree Regressor]***')
        st.write('- The Decision Tree Regressor is a supervised machine learning algorithm that predicts continuous target variables by modeling the data as a tree structure. It recursively splits the data into subsets based on feature values, creating branches that represent decision paths.')
        st.write('#### CLASSIFICATION - ***:violet[Decision Tree Classifier]***')
        st.write('- The Decision Tree Classifier is a supervised machine learning algorithm used for categorical target variables. Similar to the regressor, it partitions the dataset into subsets based on feature values to classify instances.')
...
if selected == 'CONCLUSION': 
    ...
    st.write('This project underscores the significance of leveraging machine learning in traditional industries. By transforming raw data into actionable insights, organizations in the copper sector can streamline operations, optimize pricing strategies, and make data-driven decisions that can significantly impact profitability and sustainability.The success of this application paves the way for further innovations in predictive analytics within the manufacturing sector, ultimately contributing to more effective business practices and better resilience against market volatility.')
    

st.write( f'<h6 style="color:rgb(0, 153, 153,0.35);">App Created by Dhanuja</h6>', unsafe_allow_html=True )